...
export default function TrailMissionPage() {
  ...
  const loadMission = async () => {
    ...
    try {
      ...
      if (missionData.completed) {
        toast.success('Missão concluída! Você completou essa trilha 🎉');
      }
    } catch {
      setTrail(null);
      setMission(null);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <PageHeader title="Trilha para ganhar brindes!!" />
        ...
        <div className="max-w-lg mx-auto space-y-4">
          <section className="bg-card border-2 border-primary rounded-xl p-4">
            <div className="flex items-center gap-3 text-primary">
              ...
              <div>
                <h1 className="font-bold text-foreground">Sua missão</h1>
                <p className="text-sm text-muted-foreground">Passe em todos os stands da rota e ganhe brindes.</p>
              </div>
            </div>
          </section>
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">Selecione uma categoria no mapa para gerar uma trilha.</p>
          ) : !trail || !mission ? (
            <div className="bg-card border-2 border-border rounded-xl p-4">
              <p className="text-sm text-muted-foreground">Nenhuma trilha ativa encontrada para a categoria {category}.</p>
            </div>
          ) : (
            <>
              <section className="bg-card border-2 border-border rounded-xl p-4 space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Progresso: {mission.visitedStandIds.length}/{mission.stands.length} stands
                </p>
                {mission.completed && (
                  <p className="text-sm font-semibold text-green-600">Missão concluída! 🎉</p>
                )}
              </section>
