        ...
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <section className="bg-card border-2 border-border rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2 text-primary">
              ...
              <h2 className="font-bold">Trilhas realizadas</h2>
            </div>
            ...
            {loadingTrails ? (
              ...
              <p className="text-sm text-muted-foreground">
                Você ainda não concluiu trilhas. Vá ao mapa e escolha uma categoria para começar.
              </p>
            ) : (
                ...
                {completedTrails.map((item) => (
                  <article key={item.id} className="border border-border rounded-lg p-3">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Concluída em {new Date(item.completedAt).toLocaleDateString()}
                    </p>
                    <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                      <CheckCircle2 className="w-4 h-4 text-green-600" />
                      {item.trail.stands.length} stands concluídos
                    </div>
                  </article>
                ))}
              </div>
